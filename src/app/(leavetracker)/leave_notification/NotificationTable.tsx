...
import type { getAllNotificationType } from "@/lib/queries/getAllNotification";
import {
  createColumnHelper,
  flexRender,
  getCoreRowModel,
...
type Props = {
  data: getAllNotificationType;
};

type RowType = getAllNotificationType[0];
  ...
  columnHelper.accessor((row) => {
    ...
    cell: ({ getValue }) => {
      ...
      if (typeof value === "string" && value.length > 100) {
        return (
          <span title={value} className="pointer-events-none">
            {value.slice(0, 100)}...
          </span>
          ...
          <TableBody>
            {table.getRowModel().rows.map((row) => (
              <TableRow
                ...
                onClick={() => {
                  console.log("Row data:", row.original); // <--- Check this
                  router.push(`/leave_notification/form?notificationId=${row.original.id}`);
                }}
